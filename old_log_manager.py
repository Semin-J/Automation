...
root_path, extracted_path, stat_path, archive_path, archive_temp_path = ""
    ...
    try:
        for server in range(0, 2, 1):
            for sub in range(0, 3, 1):
                os.makedirs(
                    os.path.join(target_path, server_list[server], sub_list[0][sub]),
                    access_right,
                    exist_ok=True
                )
        ...
        for server in range(2, 4, 1):
            for sub in range(0, 5, 1):
                os.makedirs(
                    os.path.join(target_path, server_list[server], sub_list[1][sub]),
                    access_right,
                    exist_ok=True
                )
                ...
                if len(origin_file_list) != 0:
                    for file in origin_file_list:
                        move(
                            ...
                            os.path.join(
                                target_path, server_list[server], sub_list[0][sub], file
                            )
                        )
                    debug_log.debug(
                        "All files in %s are moved to %s successfully!",
                        origin_file_path,
                        target_file_path,
                    )
            ...
            for sub in range(0, 5, 1):
                ...
                if len(origin_file_list) != 0:
                    for file in origin_file_list:
                        move(
                            os.path.join(
                                origin_path, server_list[server], sub_list[1][sub], file
                            ),
                            os.path.join(
                                target_path, server_list[server], sub_list[1][sub], file
                            ),
                        )
