...
def change_folder(server, sub, is_mobile):
  ...
  landing_path = (root_path + "landing/") if is_mobile == False else (root_path + "landing/mobile/")
  paths = {}
  paths['landing_path'] = (landing_path + server + sub)
  paths['extratced_path'] = (landing_path + server + sub)
  paths['stat_path'] = (stat_path + server + sub)
  paths['archive_temp_path'] = (archive_temp_path + server + sub)
  return paths
...
def unzip_move(paths):
  ...
  if len(file_list) != 0:
    ...
    for file in file_list:
      ...
      tar = tarfile.open(paths['landing_path'] + file)
      ...
      move(paths['landing_path'] + file, paths['archive_temp_path'] + file)
    # end of for
